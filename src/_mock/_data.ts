...
export const _users = [...Array(24)].map((_, index) => ({
  ...
  // new fields for simplified user table
  email: `${_fullName(index).toLowerCase().replace(/[^a-z0-9]+/g, '.') }@example.com`,
  phoneNumber: `+84 9${String(10000000 + index).slice(-8)}`,
  address: `${_company(index)}, District ${((index % 10) + 1)}`,
  totalSpent: Math.round(_price(index) * 100),
}));
